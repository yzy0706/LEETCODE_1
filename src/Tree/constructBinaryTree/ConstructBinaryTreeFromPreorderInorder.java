...
public class ConstructBinaryTreeFromPreorderInorder {
    ...
    //做法: 在preorder上的顺序是root, 左边的子树的所有点, 右边的子树的所有点; 在inorder上的顺序是左子树所有点, 根节点, 右子树, 所以我们知道在inorder上, 如果root是inorder[i]的话, 0到i-1就是他的左子树, i+1到inorder.length-1就是他的右子树, 所以我们就这么循环下去建立整个binary tree
    ...
    private TreeNode buildSubTree(int preIndex, int inStart, int inEnd, int[] preorder, int[] inorder){
        ...
        root.left = buildSubTree(preIndex + 1, inStart, inIndex - 1, preorder, inorder); //inIndex的左边建左树
        root.right = buildSubTree(preIndex + inIndex - inStart + 1, inIndex + 1, inEnd, preorder, inorder); //inIndex的右边建右子树, preorder里右子树的根节点应该是preIndex+整个左子树的大小才能越过左子树
        return root;
