...
public class LargestDivisibleSubset {
    ...
    // 3. 现在curDp = dp[maxIndex]就是我们答案要找的连续整除数列的数的个数, curNum = nums[maxIndex]就是答案的最右边最大的那个数, 一直往左边forloop,
    // 找到每一个dp[i]正好是递减的curDp的i, 而且他还要能被curNum整除, 加入到res里, 并return res;
        ...
        for(int i = 1; i < nums.length; i++){
            for(int j = i-1; j >= 0; j--){
                if(nums[i] % nums[j] == 0){
                    ...
                    //从每一个i的左边一个j往左浏览, 如果能整除这个j证明i可以成为连续整除的subset的最右边一个, 更新dp[i]
                }
            }
        }
