...
————————————————————————————————————————————————————————————————————————————————————————————————————
  ...
  出现的bug：
  1. x对应的是 matrix.length, y 对应 matrix[0].length;
...
----------------------------------------------------------------------------------------------------
Graph（记得优先建数据结构： 两种图的表示方式 ： adjacent matrix, adjacent list)
1.最短路径的三大算法：
   a. Graph:
...
----------------------------------------------------------------------------------------------------
Matrix
1. l = w > 0? matrix[0].length : 0;
2. 计算能找到的最大边长长度的时候最好沿着对角线找
3. 知道整个matrix m*n的总数的值，则去找特定的坐标是 matrix[cur/n][cur%n]
